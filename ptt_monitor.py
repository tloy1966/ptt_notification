...
def fetch_ptt_page_with_retry(url, max_retries=3):
    """
    使用重試機制取得 PTT 頁面
...
def get_ptt_articles(board, max_pages=1):
    """
    爬取 PTT 指定看板的文章
    
    Args:
        board: 看板名稱
        max_pages: 要爬取的頁數（預設為 1）
    ...
    for page_num in range(max_pages):
        ...
        response = fetch_ptt_page_with_retry(current_url)
        if not response:
...
def fetch_article_datetime(article_url):
    ...
    """
    response = fetch_ptt_page_with_retry(article_url, max_retries=2)
    if not response:
...
def main():
    ...
    pages_str = os.environ.get('PTT_PAGES', '1')
    try:
        pages = int(pages_str)
    except ValueError:
        print(f"錯誤：PTT_PAGES 必須是整數，目前是：{pages_str}")
        sys.exit(1)
    if pages < 1:
        print("錯誤：PTT_PAGES 不能小於 1")
        sys.exit(1)
    if pages > 5:
        print("警告：PTT_PAGES 設定過大，限制為最多 5 頁")
        pages = 5
    ...
    articles = get_ptt_articles(board, max_pages=pages)
